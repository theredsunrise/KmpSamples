...
class CalendarFullAccessIos17Handler() : SystemPermissionHandler {
    ...
    override fun canHandle(simplePlatformPermission: PlatformSinglePermission): Boolean {
        fun isSupported(permission: Permission): Boolean = when (permission) {
            is CalendarReadPermission -> isAtLeastIOS(17)
            else -> false
        }
    ...
    private fun createSimplePermissionState(
        ...
        val permissionState =
            PermissionState(CalendarReadPermission(), isGranted, shouldShowRationale)
    ...
    override fun requestPermission(onRequest: (permissionState: PermissionStateType) -> Unit) {
        eventStore.requestFullAccessToEventsWithCompletion { isGranted, error ->
            mainScope.launch {
